  ...
  if (!data) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div className="bg-[#0d0a07] border border-orange-900/30 rounded-2xl w-full max-w-2xl p-8 text-center text-orange-700 text-sm">
          Loading report...
        </div>
      </div>
    )
      ...
      <div className="bg-[#0d0a07] border border-orange-900/30 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-orange-900/30">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-orange-700 font-mono">{data.date}</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="card p-4 text-center">
              ...
              <div className="text-[10px] text-orange-700 mt-1 font-bold uppercase tracking-[0.2em]">Opened Today</div>
            </div>
            <div className="card p-4 text-center">
              <div className="text-3xl font-black text-orange-400 tabular-nums">{data.resolved_today}</div>
              <div className="text-[10px] text-orange-700 mt-1 font-bold uppercase tracking-[0.2em]">Resolved Today</div>
            </div>
          </div>

          {/* By priority */}
          <div className="card p-4">
            <p className="text-[10px] font-bold text-orange-700 uppercase tracking-[0.2em] mb-4">Open by Priority</p>
            <div className="flex flex-col gap-3">
              ...
              {data.open_by_priority.length === 0 && <p className="text-sm text-orange-700">No open issues.</p>}
            </div>
          </div>

          {/* By type */}
          <div className="card p-4">
            <p className="text-[10px] font-bold text-orange-700 uppercase tracking-[0.2em] mb-4">Open by Type</p>
            <div className="flex flex-col gap-3">
              {data.open_by_type.map(({ type, count }) => (
                <div key={type} className="flex items-center gap-3">
                  <span className="text-xs text-orange-300/50 w-36 truncate shrink-0">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="card p-4">
              <p className="text-[10px] font-bold text-orange-700 uppercase tracking-[0.2em] mb-3 flex items-center gap-1.5">
              ...
              {data.top_skus.slice(0, 5).map(({ sku, count }) => (
                <div key={sku} className="flex items-center justify-between py-1.5">
                  ...
                  <span className="text-xs text-orange-700 tabular-nums font-mono">{count}</span>
                </div>
              ))}
              {data.top_skus.length === 0 && <p className="text-xs text-orange-900">None</p>}
            </div>
            <div className="card p-4">
              <p className="text-[10px] font-bold text-orange-700 uppercase tracking-[0.2em] mb-3 flex items-center gap-1.5">
              ...
              {data.top_bins.length === 0 && <p className="text-xs text-orange-900">None</p>}
            </div>
          </div>
            ...
            <div className="card p-4 border-red-500/20">
              ...
              <div className="flex flex-col gap-2">
                {data.unresolved_critical.map(d => (
                  <div key={d.id} className="flex items-center gap-3 py-2 border-b border-orange-900/30 last:border-0">
                    <span className="font-mono text-xs text-orange-300/50">{d.order_number}</span>
                    ...
                    {d.assigned_name && <span className="text-xs text-orange-700">{d.assigned_name}</span>}
                  </div>
                ))}
              </div>
            </div>
