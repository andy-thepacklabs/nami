...
export default function FinalePanel({ onClose, onSync }: { onClose: () => void; onSync?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#0d0a07] border border-orange-900/30 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-orange-900/30">
          <div className="flex items-center gap-3">
            ...
            {connectionOk === true && <span className="badge text-[10px] text-orange-400 bg-orange-500/10 border-orange-500/20">Connected</span>}
        ...
        {/* Action bar */}
        <div className="flex items-center gap-3 px-6 py-3 bg-[#12100d] border-b border-orange-900/30">
        ...
        {syncResult && (
          <div className={cn(
            ...
            <div className="grid grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
              <div><span className="text-orange-700">Products:</span> <span className="font-bold text-white">{syncResult.products}</span></div>
              <div><span className="text-orange-700">Facilities:</span> <span className="font-bold text-white">{syncResult.facilities}</span></div>
              <div><span className="text-orange-700">Transfers:</span> <span className="font-bold text-white">{syncResult.transfers}</span></div>
              <div><span className="text-orange-700">Stock Levels:</span> <span className="font-bold text-white">{syncResult.stockLevels}</span></div>
              <div><span className="text-orange-700">Shipments:</span> <span className="font-bold text-white">{syncResult.shipments}</span></div>
              <div><span className="text-orange-700">Orders:</span> <span className="font-bold text-white">{syncResult.orders}</span></div>
            </div>
                ...
                <div className="flex flex-col gap-1">
                  {syncResult.detection.rules.map((r, i) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      <span className="text-orange-300/50">{r.rule}</span>
                      <span className="text-white font-mono">
                        {r.created > 0 && <span className="text-red-400">{r.created} new</span>}
                        {r.created > 0 && r.skipped > 0 && <span className="text-orange-900"> · </span>}
                        {r.skipped > 0 && <span className="text-orange-700">{r.skipped} existing</span>}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-orange-900/30 px-6 mt-2">
          {([
...
function OverviewTab({ stats }: { stats: SyncStats | null }) {
  if (!stats) return <p className="text-orange-700 text-sm">Loading...</p>

  const cards = [
    { label: 'Products', value: stats.products, icon: <Package className="w-5 h-5" />, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Locations / Bins', value: stats.facilities, icon: <MapPin className="w-5 h-5" />, color: 'text-orange-400', bg: 'bg-orange-500/10' },
    ...
    <div className="flex flex-col gap-6">
      ...
      {stats.lastSync ? (
        <div className="card p-4">
          <p className="text-[10px] font-bold text-orange-700 uppercase tracking-[0.2em] mb-3">Last Sync Details</p>
        ...
        <div className="card p-8 text-center">
          <Database className="w-8 h-8 text-orange-900 mx-auto mb-3" />
          <p className="text-sm text-orange-300/50 mb-1">No data synced yet</p>
          <p className="text-xs text-orange-900">Click "Full Sync" above to pull your inventory from Finale.</p>
        </div>
      )}
    </div>
  )
...
function Row({ label, value, error }: { label: string; value: string; error?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 py-1">
      <span className="text-xs text-orange-700">{label}</span>
...
function DataTable({ endpoint, columns }: { endpoint: string; columns: string[] }) {
  ...
  if (error && rows.length === 0) {
    return (
      <div className="card p-8 text-center">
        <Database className="w-8 h-8 text-orange-900 mx-auto mb-3" />
        <p className="text-sm text-orange-300/50">{error}</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-xs text-orange-700 font-medium">{total} record{total !== 1 ? 's' : ''}</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-orange-900/30 bg-[#12100d]">
                {columns.map(col => (
                  <th key={col} className="text-left text-[10px] font-bold text-orange-700 px-4 py-3 uppercase tracking-[0.15em] whitespace-nowrap">
                    {col.replace(/_/g, ' ')}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-orange-900/20/50">
              {loading ? (
                <tr><td colSpan={columns.length} className="px-4 py-8 text-center text-orange-700 text-xs">Loading...</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={columns.length} className="px-4 py-8 text-center text-orange-700 text-xs">No data found</td></tr>
              ) : (
                rows.map((row, i) => (
                  <tr key={i} className="hover:bg-orange-950/40 transition-colors">
                    {columns.map(col => (
                      <td key={col} className="px-4 py-2.5 text-xs text-orange-200/70 font-mono whitespace-nowrap">
                        {String(row[col] ?? '—')}
                      </td>
                    ))}
                  </tr>
