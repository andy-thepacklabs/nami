...
export default function FinalePanel({ onClose, onSync }: { onClose: () => void; onSync?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#0d1117] border border-[#1e2433] rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
        {/* Header */}
        ...
        {syncResult && (
          <div className={cn(
            ...
            <div className="grid grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
              <div><span className="text-slate-500">Products:</span> <span className="font-bold text-white">{syncResult.products}</span></div>
              <div><span className="text-slate-500">Facilities:</span> <span className="font-bold text-white">{syncResult.facilities}</span></div>
              <div><span className="text-slate-500">Transfers:</span> <span className="font-bold text-white">{syncResult.transfers}</span></div>
              <div><span className="text-slate-500">Stock Levels:</span> <span className="font-bold text-white">{syncResult.stockLevels}</span></div>
              <div><span className="text-slate-500">Shipments:</span> <span className="font-bold text-white">{syncResult.shipments}</span></div>
              <div><span className="text-slate-500">Orders:</span> <span className="font-bold text-white">{syncResult.orders}</span></div>
            </div>
                  ...
                  {syncResult.detection.rules.map((r, i) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-white font-mono">
                        ...
                        {r.created > 0 && r.skipped > 0 && <span className="text-slate-600"> · </span>}
                        {r.skipped > 0 && <span className="text-slate-500">{r.skipped} existing</span>}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-[#1e2433] px-6 mt-2">
          {([
    ...
    <div className="flex flex-col gap-6">
        ...
        {cards.map(({ label, value, icon, color, bg }) => (
          <div key={label} className="card p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.15em]">{label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {stats.lastSync ? (
        <div className="card p-4">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3">Last Sync Details</p>
        ...
        <div className="card p-8 text-center">
          ...
          <p className="text-sm text-slate-400 mb-1">No data synced yet</p>
          <p className="text-xs text-slate-600">Click "Full Sync" above to pull your inventory from Finale.</p>
        </div>
      )}
    </div>
  )
...
function DataTable({ endpoint, columns }: { endpoint: string; columns: string[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-xs text-slate-500 font-medium">{total} record{total !== 1 ? 's' : ''}</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1e2433] bg-[#111520]">
                {columns.map(col => (
                  <th key={col} className="text-left text-[10px] font-bold text-slate-500 px-4 py-3 uppercase tracking-[0.15em] whitespace-nowrap">
                    {col.replace(/_/g, ' ')}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {loading ? (
                <tr><td colSpan={columns.length} className="px-4 py-8 text-center text-slate-500 text-xs">Loading...</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={columns.length} className="px-4 py-8 text-center text-slate-500 text-xs">No data found</td></tr>
              ) : (
                rows.map((row, i) => (
                  <tr key={i} className="hover:bg-[#1a1f2e] transition-colors">
                    {columns.map(col => (
                      <td key={col} className="px-4 py-2.5 text-xs text-slate-300 font-mono whitespace-nowrap">
                        {String(row[col] ?? '—')}
                      </td>
                    ))}
                  </tr>
